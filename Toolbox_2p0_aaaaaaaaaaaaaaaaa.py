...
# function for sort cells of all recordings of one fish layer
def sort_cells_advanced(files, all_time_sorted_cells, all_sorted_cells_idx):
    # put time sorted cells into one single array
    total_cells = int(np.sum(np.shape(all_time_sorted_cells[i])[0] for i in files))
    # identify maximum size of all cell traces
    max_time = int(np.max([np.shape(all_time_sorted_cells[i])[1] for i in range(len(all_time_sorted_cells))]))
    # pre allocation
    plot_cells = np.zeros((total_cells, max_time))
    all_sort = [[] for _ in range(40)]
    # iterate over desired files and sort condition after condition in cells
    cell_counter = 0
    for file in files:
        current = all_time_sorted_cells[file]
        current_sort = all_sorted_cells_idx[file]
        for cond in range(np.shape(current_sort)[0]):
            current_cond = current_sort[cond, ~np.isnan(current_sort[cond, :])].astype("int64")
            for cell, trace in enumerate(current[current_cond]):
                plot_cells[cell_counter, :len(trace)] = trace
                cell_counter += 1
            all_sort[cond].extend(current_cond)
    # rewrite as array
    max_size = max([len(all_sort[i]) for i in range(len(all_sort))])

    placeholder = np.zeros((40, max_size), dtype="int64")
    for idx, sort in enumerate(all_sort):
        placeholder[idx, :len(sort)] = sort[:]
    all_sort = placeholder
    return plot_cells, all_sort

# function for pixel plot
def pixel_plot(plot_cells, frame_rate, conditions, line_counter, extracted_regressors,
               all_sort, colour, cmap="hot", clim_factor=0.5, size=20, linewidth=2, figwidth=30, figheight=20):
    # pre allocation
    show_line = False
    pixel_ticks = []
    trace_ticks = []
    ...
    trace_label = []
    # redefine time axis
    time = (np.linspace(0, np.shape(plot_cells)[1] / frame_rate)).astype("int64")
    x = np.arange(0, np.shape(extracted_regressors)[1])
    dot_interval = [[0], []]
    # get list of conditions
    split_all = [i.split("_") for i in conditions]
    # get amount of cells for each condition
    cell_interval = np.zeros((np.shape(all_sort)[0] + 1), dtype="int64")
    amount_cells = int(np.shape(plot_cells)[0])
    for i in range(np.shape(all_sort)[0]):
        cell_interval[i] = amount_cells
        amount_cells -= np.count_nonzero(all_sort[i])
    ...
    with ((plt.rc_context({"font.size": size, "axes.titlesize": size + size*.2, "axes.labelsize": size + size*.2,
        ...
        # plot pixelplot
        im = pixelplot.imshow(plot_cells[::-1], cmap=cmap, origin="lower")
        im.set_clim(vmin=np.nanmin(plot_cells), vmax=np.nanmax(plot_cells) * clim_factor)
        ...
        for line in range(len(conditions)):
            ...
            if show_line:
                pixelplot.axvline(line_counter[line], ymin=0, ymax=np.shape(plot_cells)[0], linestyle="--",
                                  color="white", linewidth=linewidth)
                pixelplot.axhline(cell_interval[line + 1], xmin=0, xmax=np.shape(plot_cells)[1], linestyle="--",
                                  color="white", linewidth=linewidth)
                regressor_plot.axvline(line_counter[line], ymin=0, ymax=np.nanmax(extracted_regressors) + 1, linestyle="--",
                                  color="white", linewidth=linewidth)
            ...
            regressor_plot.plot(x, extracted_regressors[line, :], color=colour[line, :], linewidth=linewidth)
        ...
        # adjust the plot
        regressor_plot.axhline(0, xmin=0, xmax=np.shape(extracted_regressors)[1], color="black", linewidth=linewidth)
        ...
        pixelplot.set_ylim([0, np.shape(plot_cells)[0]])
        pixelplot.set_ylabel("Cells", labelpad=20, weight="bold")

        # move ticks to center of window (Credits to Carina)
        pixel_ticks.append(
            int(np.shape(plot_cells)[1]) + (int(np.shape(plot_cells)[1]) - pixel_ticks[-1]) * 2)
        pixel_labels.append(" ")
        pixel_yticks.insert(0, int(np.shape(plot_cells)[0]))
        ...
        y2.set_ylim([0, np.shape(plot_cells)[0]])
        y2.set_yticks(ticks=np.array(np.array(pixel_yticks[:]) - np.diff([0] + pixel_yticks[:]) / 2)[1:],
                      labels=pixel_labels[:-1], weight="bold")
    return fig

# function to create plot of dff + regressor trace
def plot_regressor(dff, t_min, frame_rate, random_cells, all_regressors_conv, cell_sorting,
                   amount_plots=10, size=20, linewidth=2, figheight=30, figwidth=20):
    # define time axis
    time_ax = np.linspace(0, (np.shape(dff)[1] - t_min)/frame_rate, np.shape(dff)[1] - t_min, endpoint=True).astype("int64")
    ...
    with ((plt.rc_context({"font.size": size, "axes.titlesize": size, "axes.labelsize": size, "ytick.labelsize": size,
        ...
        for subplt in range(amount_plots):
            current_cell = random_cells[subplt]
            ax = axs[subplt]
            # plot regressor trace
            reg = ax.plot(time_ax, all_regressors_conv[cell_sorting[current_cell]][plot_indices], color="tab:pink",
                          linewidth=linewidth, label="Regressor")
            # plot dff trace
            trace = ax.plot(time_ax, dff[current_cell, plot_indices], color="tab:blue", linewidth=linewidth,
                            label="dFF", alpha=1)
